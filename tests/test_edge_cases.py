...
class TestExceptionHandling:
    ...
    def test_exception_wrapping(self):
        ...
        try:
            raise RuntimeError(f"Error communicating with API: {original_error}") from original_error
...
class TestConfigFlowInit:
    ...
    async def test_config_flow_can_be_initiated(self, hass):
        ...
        assert result["type"] == FlowResult.FORM["type"] if hasattr(FlowResult, "FORM") else "form"
    ...
    async def test_config_flow_shows_form(self, hass):
        ...
        assert "data_schema" in result or "errors" in result or result.get("type") == "form"
