...
from custom_components.mysa.const import (
    DOMAIN,
    AC_MODE_OFF, AC_MODE_AUTO, AC_MODE_HEAT, AC_MODE_COOL, AC_MODE_DRY, AC_MODE_FAN_ONLY
)
...
def mock_api():
    ...
    api.get_devices = AsyncMock(return_value={
        ...
        "ac_device": {"Id": "ac_device", "Name": "Bedroom AC", "Model": "AC-V1", "SupportedCaps": {"modes": {"4": {}}}},
        "lite_device": {"Id": "lite_device", "Name": "Office", "Model": "BB-V2-0-L"},
    })
    return api
...
class TestMysaClimateInit:
    ...
    @pytest.mark.asyncio
    async def test_climate_init(self, hass, mock_coordinator, mock_device_data, mock_api, mock_entry):
    ...
    @pytest.mark.asyncio
    async def test_climate_device_info(self, hass, mock_coordinator, mock_device_data, mock_api, mock_entry):
...
class TestMysaClimateProperties:
    ...
    @pytest.fixture
    def climate_entity(self, hass, mock_coordinator, mock_device_data, mock_api, mock_entry):
...
class TestMysaClimateActions:
    ...
    @pytest.mark.asyncio
    async def test_set_temperature(self, hass, mock_coordinator, climate_entity, mock_api):
    ...
    @pytest.mark.asyncio
    async def test_set_hvac_mode_off(self, hass, mock_coordinator, climate_entity, mock_api):
    ...
    @pytest.mark.asyncio
    async def test_set_hvac_mode_heat(self, hass, mock_coordinator, climate_entity, mock_api):
...
class TestMysaACClimateInit:
    ...
    @pytest.mark.asyncio
    async def test_ac_climate_init(self, hass, mock_coordinator, mock_ac_device_data, mock_api, mock_entry):
...
class TestMysaACClimateProperties:
    ...
    @pytest.fixture
    def ac_entity(self, hass, mock_coordinator, mock_ac_device_data, mock_api, mock_entry):
...
class TestMysaACClimateActions:
    ...
    @pytest.mark.asyncio
    async def test_set_hvac_mode_cool(self, hass, mock_coordinator, ac_entity, mock_api):
...
class TestClimateEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_hvac_mode_off(self, hass, mock_coordinator_off, mock_device_data, mock_api, mock_entry):
        """Test hvac_mode returns OFF when mode is 1."""
        from custom_components.mysa.climate import MysaClimate
        
        await mock_coordinator_off.async_refresh()
        
        entity = MysaClimate(mock_coordinator_off, "off_device", mock_device_data, mock_api, mock_entry)
        
        assert entity.hvac_mode == HVACMode.OFF

    @pytest.mark.asyncio
    async def test_hvac_action_off(self, hass, mock_coordinator_off, mock_device_data, mock_api, mock_entry):
    ...
    async def test_hvac_action_idle(self, hass, mock_device_data, mock_api, mock_entry):
        ...
        from custom_components.mysa.climate import MysaClimate
        entity = MysaClimate(coordinator, "idle_device", mock_device_data, mock_api, mock_entry)
        
        assert entity.hvac_action == HVACAction.IDLE

    @pytest.mark.asyncio
    async def test_no_state_properties(self, hass, mock_coordinator_no_state, mock_device_data, mock_api, mock_entry):
        """Test properties return None when no state data."""
        from custom_components.mysa.climate import MysaClimate
        
        await mock_coordinator_no_state.async_refresh()
        
        entity = MysaClimate(mock_coordinator_no_state, "missing_device", mock_device_data, mock_api, mock_entry)
    ...
    @pytest.mark.asyncio
    async def test_extract_value_with_id_fallback(self, hass, mock_coordinator, mock_device_data, mock_api, mock_entry):
        """Test _extract_value falls back to 'Id' key in dict."""
        from custom_components.mysa.climate import MysaClimate
        
        await mock_coordinator.async_refresh()
        
        entity = MysaClimate(mock_coordinator, "device1", mock_device_data, mock_api, mock_entry)
...
class TestACClimateEdgeCases:
    ...
    def mock_coordinator_ac_modes(self, hass):
        ...
        async def async_update():
            return {
                ...
                "ac_auto": {"md": 2, "ambTemp": 25.0, "stpt": 22.0},  # Auto (cooling needed)
                "ac_auto_heat": {"md": 2, "ambTemp": 18.0, "stpt": 22.0},  # Auto (heating needed)
                "ac_off": {"md": 1},  # Off
            }
        return DataUpdateCoordinator(hass, MagicMock(), name="test", update_method=async_update)
    ...
    def ac_device_data(self):
        ...
        return {
            ...
            "Name": "Test AC",
            "Model": "AC-V1",
            "SupportedCaps": {"modes": {"4": {"fanSpeeds": [0, 1], "verticalSwing": [0, 1]}}}
        }

    @pytest.mark.asyncio
    async def test_ac_hvac_action_heat(self, hass, mock_coordinator_ac_modes, ac_device_data, mock_api, mock_entry):
        """Test AC hvac_action returns HEATING."""
        from custom_components.mysa.climate import MysaACClimate
        
        await mock_coordinator_ac_modes.async_refresh()
        
        entity = MysaACClimate(mock_coordinator_ac_modes, "ac_heat", ac_device_data, mock_api, mock_entry)
        
        assert entity.hvac_action == HVACAction.HEATING

    @pytest.mark.asyncio
    async def test_ac_hvac_action_dry(self, hass, mock_coordinator_ac_modes, ac_device_data, mock_api, mock_entry):
        """Test AC hvac_action returns DRYING."""
        from custom_components.mysa.climate import MysaACClimate
        
        await mock_coordinator_ac_modes.async_refresh()
        
        entity = MysaACClimate(mock_coordinator_ac_modes, "ac_dry", ac_device_data, mock_api, mock_entry)
        
        assert entity.hvac_action == HVACAction.DRYING

    @pytest.mark.asyncio
    async def test_ac_hvac_action_fan(self, hass, mock_coordinator_ac_modes, ac_device_data, mock_api, mock_entry):
        """Test AC hvac_action returns FAN."""
        from custom_components.mysa.climate import MysaACClimate
        
        await mock_coordinator_ac_modes.async_refresh()
        
        entity = MysaACClimate(mock_coordinator_ac_modes, "ac_fan", ac_device_data, mock_api, mock_entry)
        
        assert entity.hvac_action == HVACAction.FAN

    @pytest.mark.asyncio
    async def test_ac_hvac_action_auto_cooling(self, hass, mock_coordinator_ac_modes, ac_device_data, mock_api, mock_entry):
        """Test AC hvac_action returns COOLING when auto mode and current > target."""
        from custom_components.mysa.climate import MysaACClimate
        
        await mock_coordinator_ac_modes.async_refresh()
        
        entity = MysaACClimate(mock_coordinator_ac_modes, "ac_auto", ac_device_data, mock_api, mock_entry)
        
        assert entity.hvac_action == HVACAction.COOLING

    @pytest.mark.asyncio
    async def test_ac_hvac_action_auto_heating(self, hass, mock_coordinator_ac_modes, ac_device_data, mock_api, mock_entry):
        """Test AC hvac_action returns HEATING when auto mode and current < target."""
        from custom_components.mysa.climate import MysaACClimate
        
        await mock_coordinator_ac_modes.async_refresh()
        
        entity = MysaACClimate(mock_coordinator_ac_modes, "ac_auto_heat", ac_device_data, mock_api, mock_entry)
        
        assert entity.hvac_action == HVACAction.HEATING

    @pytest.mark.asyncio
    async def test_ac_hvac_action_off(self, hass, mock_coordinator_ac_modes, ac_device_data, mock_api, mock_entry):
        """Test AC hvac_action returns OFF."""
        from custom_components.mysa.climate import MysaACClimate
        
        await mock_coordinator_ac_modes.async_refresh()
        
        entity = MysaACClimate(mock_coordinator_ac_modes, "ac_off", ac_device_data, mock_api, mock_entry)
        
        assert entity.hvac_action == HVACAction.OFF

    @pytest.mark.asyncio
    async def test_ac_turn_on(self, hass, mock_coordinator, mock_ac_device_data, mock_api, mock_entry):
        """Test AC async_turn_on sets to cool mode."""
        from custom_components.mysa.climate import MysaACClimate
        
        await mock_coordinator.async_refresh()
        
        entity = MysaACClimate(mock_coordinator, "ac_device_123", mock_ac_device_data, mock_api, mock_entry)
        entity.async_write_ha_state = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_ac_set_hvac_mode_all_modes(self, hass, mock_coordinator, mock_ac_device_data, mock_api, mock_entry):
        ...
        for mode in [HVACMode.OFF, HVACMode.HEAT, HVACMode.COOL, HVACMode.HEAT_COOL, HVACMode.FAN_ONLY, HVACMode.DRY]:
            mock_api.reset_mock()
            await entity.async_set_hvac_mode(mode)
            mock_api.set_hvac_mode.assert_called()

    @pytest.mark.asyncio
    async def test_ac_no_state_defaults(self, hass, mock_ac_device_data, mock_api, mock_entry):
        ...
        from custom_components.mysa.climate import MysaACClimate
        entity = MysaACClimate(coordinator, "missing", mock_ac_device_data, mock_api, mock_entry)
...
class TestClimateExceptionHandling:
    ...
    @pytest.mark.asyncio
    async def test_set_temperature_exception(self, hass, mock_coordinator, mock_device_data, mock_entry):
    ...
    @pytest.mark.asyncio
    async def test_set_hvac_mode_exception(self, hass, mock_coordinator, mock_device_data, mock_entry):
    ...
    @pytest.mark.asyncio
    async def test_ac_set_fan_mode_exception(self, hass, mock_coordinator, mock_ac_device_data, mock_entry):
    ...
    @pytest.mark.asyncio
    async def test_ac_set_swing_mode_exception(self, hass, mock_coordinator, mock_ac_device_data, mock_entry):
...
class TestACClimateEdgeCases2:
    ...
    def mock_ac_coordinator(self, hass):
        ...
        coord = DataUpdateCoordinator(hass, MagicMock(), name="test", update_method=async_update)
        coord.data = data
        return coord
    ...
    def mock_ac_entity(self, hass, mock_ac_coordinator, mock_ac_entry):
        ...
        return MysaACClimate(mock_ac_coordinator, "ac_device", device_data, mock_api, mock_ac_entry)
    ...
    async def test_ac_swing_mode_fallback(self, mock_ac_entity):
        ...
        data = mock_ac_entity.coordinator.data["ac_device"]
        if "ss" in data: del data["ss"]
        data["SwingMode"] = "vertical"
        
        assert mock_ac_entity.swing_mode == "vertical"

    @pytest.mark.asyncio
    async def test_ac_climate_swing_modes_append(self, hass, mock_ac_coordinator, mock_ac_entry):
        ...
        mock_api.get_devices = AsyncMock(return_value={
            "ac_device": {
                "Id": "ac_device", "Name": "AC", "Model": "AC-V1", 
                "SupportedCaps": {
                    "modes": {
                        "4": {  # Cool mode
        ...
        entity = MysaACClimate(mock_ac_coordinator, "ac_device", device_data, mock_api, mock_ac_entry)
...
class TestHeatingClimateEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_climate_heating_action_idle(self, hass, mock_heating_coordinator, mock_entry):
        """Test heating action returns IDLE when no state."""
        from custom_components.mysa.climate import MysaClimate
        
        mock_api = MagicMock()
        device_data = {"Id": "d1", "Name": "H", "Model": "BB-V2"}
        entity = MysaClimate(mock_heating_coordinator, "d1", device_data, mock_api, mock_entry)
        
        # Line 205 -> IDLE
        assert entity.hvac_action == HVACAction.IDLE

    @pytest.mark.asyncio
    async def test_climate_hvac_mode_fallback(self, hass, mock_heating_coordinator, mock_entry):
    ...
    @pytest.mark.asyncio
    async def test_climate_hvac_mode_unknown(self, hass, mock_heating_coordinator, mock_entry):
...
class TestSensorFinalEdgeCases:
    ...
    async def test_sensor_value_error(self, hass, mock_entry):
        ...
        entity = MysaDiagnosticSensor(coordinator, "d1", device_data, "TimeZone", "TZ", None, None, None, mock_entry)
    ...
    async def test_sensor_value_type_error(self, hass, mock_entry):
        ...
        device_data = {"d1": {"Model": "BB-V2"}}
        entity = MysaDiagnosticSensor(coordinator, "d1", device_data, "Rssi", "RSSI", "dBm", None, None, mock_entry)
    ...
    async def test_simulated_current_no_state(self, hass, mock_entry):
        ...
        entity = MysaSimulatedCurrentSensor(coordinator, "d1", device_data, 15.0, mock_entry)
    ...
    async def test_simulated_power_dict_duty(self, hass, mock_entry):
        ...
        device_data = {"Id": "d1", "Name": "L", "Model": "L"}
        entity = MysaSimulatedPowerSensor(coordinator, "d1", device_data, 15.0, mock_entry)
...
class TestClimateFinal100:
    ...
    @pytest.mark.asyncio
    async def test_current_temp_none_when_key_missing(self, hass, mock_coordinator_with_state, mock_entry):
        """Test current_temperature returns None when temp key not found."""
        from custom_components.mysa.climate import MysaClimate
        
        mock_api = MagicMock()
        device_data = {"Id": "d1", "Name": "H", "Model": "BB-V2"}
        entity = MysaClimate(mock_coordinator_with_state, "d1", device_data, mock_api, mock_entry)
        
        # Line 128 -> return None
        assert entity.current_temperature is None

    @pytest.mark.asyncio
    async def test_target_temp_none_when_key_missing(self, hass, mock_coordinator_with_state, mock_entry):
    ...
    @pytest.mark.asyncio
    async def test_humidity_none_when_key_missing(self, hass, mock_coordinator_with_state, mock_entry):
    ...
    @pytest.mark.asyncio
    async def test_set_temperature_no_temp(self, hass, mock_coordinator_with_state, mock_entry):
...
class TestClimateSetup:
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_with_heating_device(self, hass, mock_coordinator, mock_api, mock_entry):
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_creates_ac_climate(self, hass, mock_coordinator, mock_entry):
        ...
        mock_api = MagicMock()
        mock_api.get_devices = AsyncMock(return_value={
            "ac_device": {"Id": "ac_device", "Name": "AC", "Model": "AC-V1", "SupportedCaps": {"modes": {"4": {}}}}
        })
