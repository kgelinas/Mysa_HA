...
from custom_components.mysa import mysa_mqtt
...
async def test_connect_websocket_fallback(mock_connect):
    ...
    mock_connect.side_effect = [TypeError("unexpected keyword argument 'additional_headers'"), AsyncMock()]
...
async def test_mqtt_connection_enter_bad_connack(mock_ws):
    ...
    with patch('custom_components.mysa.mysa_mqtt.connect_websocket', return_value=mock_ws), \
         patch('custom_components.mysa.mysa_mqtt.parse_mqtt_packet', return_value="NotConnack"):
