...
try:
    ...
    print(
        "Note: Install prompt_toolkit for better input experience: pip install prompt_toolkit"
    )
...
class MysaReading:
    ...
    rest: bytes | None  # Trailing bytes
    ...
    def parse_readings(cls, readings: bytes) -> list[Any]:
        ...
        while offset < len(readings):
            ...
            known_vers: dict[int, type] = {
                0: MysaReadingV0,
                1: MysaReadingV1,
                3: MysaReadingV3,
            }
            reading_cls = known_vers.get(ver, cls)
            reading, offset = reading_cls._make_reading(
                kwargs, readings, offset, next_pos
    ...
    def __str__(self):
        ...
        return (
            f"[{dt}] temp={self.sensor_t:.1f}°C, amb={self.ambient_t:.1f}°C, "
...
class MysaDebugTool:
    ...
    async def authenticate(self):
        ...
        if os.path.exists(self.auth_path):
            try:
                ...
                self.session.headers["authorization"] = str(self._user_obj.id_token)  # type: ignore[union-attr]
        ...
        while True:
            try:
                if HAS_PROMPT_TOOLKIT:
                    ...
                    cmd_raw = await asyncio.get_event_loop().run_in_executor(
                        None, input, "CMD> "
                    )
    ...
    def _handle_sniff(self, parts):
        ...
        if len(parts) < 2:
            print(f"Sniff Mode is currently: {'ON' if self.sniff_mode else 'OFF'}")
            if self.sniff_filter:
                print(f"Filter: {self.sniff_filter}")
            print("Usage: sniff <ID|all|off>")
            return
        ...
        if arg == "all":
            ...
            self.sniff_filter = None
            print("Sniff Mode: ON (Global)")
            return

        # Try device ID
        did = self._resolve_device(parts[1])
        if did:
            self.sniff_mode = True
            self.sniff_filter = did
            print(f"Sniff Mode: ON (Filtered to {did})")
    ...
    def print_help(self):
        ...
        print("  quit, exit, q     Exit tool")

        print("\n[ Device Information ]")
        print("  state <ID|all>    Show full device state (HTTP + MQTT)")
        ...
        try:
            ...
            if homes:
                ...
                for i, home in enumerate(homes, 1):
                    home_id = home.get(
                        "Id", home.get("id", home.get("HomeId", "Unknown"))
                    )
    ...
    def show_examples(self):
        ...
        print(
            f'  mqtt {did} {{"cmd":[{{"sp":21,"stpt":21,"a_sp":21,"tm":-1}}],"type":4,"ver":1}}'
        )
        ...
        print(
            f'  mqtt {did} {{"cmd":[{{"tm":-1,"br":{{"a_b":1,"a_br":100,"i_br":50,'
            f'"a_dr":60,"i_dr":30}}}}],"type":4,"ver":1}}'
        )
        print(
            "       ^ Brightness (a_b=auto, a_br=active%, i_br=idle%, a_dr/i_dr=duration)"
        )

        print("\n--- MQTT AC Examples (type=2 for AC-V1) ---")
        print(
            f'  mqtt {did} {{"cmd":[{{"sp":22,"stpt":22,"tm":-1}}],"type":2,"ver":1}}'
        )
        ...
        print(
            "       ^ Set fan speed (fn: 1=Auto, 3=Low, 5=MedLow, 7=Med, 8=High, 12=Sleep)"
        )
        ...
        print(
            f"  refresh {did}        # Force device to check cloud settings (MsgType 6)"
        )
        print(
            f"  dump {did}           # Force metadata dump: FW, IP, Serial (MsgType 7)"
        )
        print()
    ...
    async def show_state(self, device_ref):
        ...
        for did in dids:
            print(f"\n{'=' * 40}")
            print(f" Fetching state for {did}...")
            print(f"{'=' * 40}")
    ...
    async def send_http(self, device_ref, json_str):
        if device_ref.lower() == "all":
            dids = list(self.devices.keys())
            print(f"Sending HTTP command to ALL {len(dids)} devices...")
        else:
            did = self._resolve_device(device_ref)
            if not did:
                return
        ...
        for did in dids:
            ...
            try:
                ...
                print(f"HTTP Request failed for {did}: {e}")
    ...
    async def send_mqtt_raw(self, device_ref, json_str):
        if device_ref.lower() == "all":
            dids = list(self.devices.keys())
            print(f"Sending MQTT command to ALL {len(dids)} devices...")
        else:
            did = self._resolve_device(device_ref)
            if not did:
                return
    ...
    async def refresh_settings(self, device_ref):
        ...
        if device_ref.lower() == "all":
            dids = list(self.devices.keys())
            print(f"Requesting settings refresh for ALL {len(dids)} devices...")
        else:
            did = self._resolve_device(device_ref)
            if not did:
                return
    ...
    async def dump_readings(self, device_ref):
        ...
        if device_ref.lower() == "all":
            dids = list(self.devices.keys())
            print(f"Requesting metadata dump for ALL {len(dids)} devices...")
        else:
            did = self._resolve_device(device_ref)
            if not did:
                return
    ...
    def _print_sniff(self, pkt):
        ...
        try:
            ...
            if msg_type == 3:
                ...
                print(
                    json.dumps(
                        {k: v for k, v in payload.items() if k != "body"}, indent=2
                    )
                )
                ...
                if readings_b64:
                    try:
                        ...
                        if parsed:
                            print(
                                f"  Version: v{parsed[0].ver} ({len(parsed)} readings)"
                            )
                            for r in parsed:
                                valid_mark = ""
                                if hasattr(r, "valid"):
                            ...
                            print(
                                f"    {' '.join(hex_str[i : i + 2] for i in range(0, len(hex_str), 2))}"
                            )
