...
def _get_mysa_device_info(
    hass: HomeAssistant, device_id: str
) -> tuple[str, ConfigEntry, Any]:
    ...
    if MYSA_DOMAIN not in hass.data:
        raise HomeAssistantError(
            translation_domain=DOMAIN, translation_key="mysa_integration_not_loaded"
        )
    ...
    if not mysa_data or not isinstance(mysa_data, dict):
        raise HomeAssistantError(
            translation_domain=DOMAIN, translation_key="mysa_data_invalid"
        )

    api = mysa_data.get("api")
    if not api:
        raise HomeAssistantError(
            translation_domain=DOMAIN, translation_key="mysa_api_not_initialized"
        )
...
async def async_service_upgrade_lite(call: ServiceCall, hass: HomeAssistant) -> None:
    ...
    try:
        ...
        if await api.async_upgrade_lite_device(mysa_device_id):
            ...
            _LOGGER.info(
                "Successfully upgraded device %s and updated core options",
                mysa_device_id,
            )
        else:
            raise HomeAssistantError(
...
async def async_service_downgrade_lite(call: ServiceCall, hass: HomeAssistant) -> None:
    ...
    try:
        ...
        if await api.async_downgrade_lite_device(mysa_device_id):
            ...
            _LOGGER.info(
                "Successfully reverted device %s and updated core options",
                mysa_device_id,
            )
        else:
            raise HomeAssistantError(
...
def async_setup_services(hass: HomeAssistant) -> None:
    ...
    hass.services.async_register(
        DOMAIN, "upgrade_lite_device", partial(async_service_upgrade_lite, hass=hass)
    )
    ...
    hass.services.async_register(
        DOMAIN, "killer_ping", partial(async_service_killer_ping, hass=hass)
    )
