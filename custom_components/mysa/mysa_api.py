"""API Client for Mysa."""
# TODO: Refactor to reduce file size
import logging
import asyncio
import json
import time
...
class MysaApi:  # TODO: Refactor MysaApi to reduce public methods and instance attributes
    """Mysa API Client."""
    ...
    def _get_state_sync(self):
        ...
        for device_id, live_data in new_states.items():
            ...
            _LOGGER.debug(
                "Merged state for %s. Keys: %s",
                device_id,
                list(self.states[device_id].keys())
            )

        return self.states

    def _normalize_state(self, state):  # TODO: Refactor _normalize_state to reduce complexity
    ...
    async def _send_mqtt_command(
            self, device_id, payload, msg_type=44, src_type=100, wrap=True
    ):  # TODO: Refactor _send_mqtt_command to reduce complexity
                        ...
                        if isinstance(pkt, mqtt.PublishPacket):
                            ...
                            if payload.get('msg') in [40, 44]:
                                ...
                                if 'lc' in state_update or 'lk' in state_update:
                                    state_update['Lock'] = state_update.get(
                                        'lc', state_update.get('lk')
                                    )
                                ...
                                _LOGGER.debug(
                                    "Updated state for %s from MQTT: %s",
                                    device_id, self.states[device_id]
                                )
    ...
    def _get_payload_type(self, device_id):  # TODO: Refactor _get_payload_type to reduce return statements
        ...
        model = (device.get("Model") or device.get("ProductModel") or
                 device.get("productModel") or "")
    ...
    async def set_hvac_mode(self, device_id, hvac_mode):
        ...
        payload_type = self._get_payload_type(device_id)
        _LOGGER.debug(
            "Using type %s for mode %s (val=%s)",
            payload_type, hvac_mode, mode_val
        )
    ...
    async def _mqtt_listen(self):
        ...
        except Exception as listen_error:  # TODO: Catch specific exceptions instead of Exception
            _LOGGER.debug(
    ...
    async def _run_mqtt_loop(self, ws):
        ...
        while True:
            try:
                ...
                except Exception as parse_error:  # TODO: Catch specific exceptions instead of Exception
                    _LOGGER.warning(
                ...
                pass
            except Exception as recv_error:  # TODO: Catch specific exceptions instead of Exception
                _LOGGER.error(
    ...
    def _resolve_device_id(self, topic):
        """Resolve device ID from MQTT topic."""
    ...
    async def _process_mqtt_publish(self, pkt):
        ...
        try:
            ...
            device_id = self._resolve_device_id(pkt.topic)
            if not device_id:
                return
